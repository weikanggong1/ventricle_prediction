# Ventricle prediction — 从零训练与独立测试
...
- 不需要已有 checkpoint。仓库不含预训练权重、原始受试者影像或原始标注。
...
已在 Python 3.11、PyTorch 2.5.1 环境验证。默认 CPU；真实训练可指定 `--device cuda:0`。
...
默认训练 50 epochs，5 epochs warmup，batch size=4，AdamW lr=1e-4，input dropout=0.25。可指定 `--epochs`、`--warmup-epochs`、`--batch-size`、`--workers`、`--lr`、`--seed`。模型保留原 notebook 的 patch=256、depth=18、embed_dim=128、heads=8；depth=18 是原 UViT 构造参数，对应 9+1+9 个 block。
...
python smoke_test.py --output smoke_run
```

已经在 gpucw1 的 CPU 上通过：

- 6 项测试：图像标准化/网格、异常输入、标签与重复 ID、模型重载、数据划分/泄漏检查、评估指标。
- 两个完整结构模型从随机初始化各训练 2 epochs；模拟样本分为 12 训练、4 验证、4 测试。
- 最佳模型在新进程重载后，独立测试输出完全一致。
- 训练数据被用作独立测试时会拒绝，并且不写出测试结果。

见 `validation/smoke_report.json` 和 `validation/unit_tests.txt`。这些验证针对代码执行链，未对真实数据重新训练或报告预测性能。
